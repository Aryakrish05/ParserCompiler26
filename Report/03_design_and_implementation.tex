...
Every path in \texttt{parse()} is required to end with \texttt{return ACCEPT} or \texttt{return REJECT}, mirroring the C convention of stating the final accept/reject decision.
...
The STMSynthesiser takes the specification produced by the SpecBuilder and synthesises the state machine. It uses a CEGIS loop and constrains the search to a tabular representation of the FSM. 
This restricted format is what allows the synthesis to remain tractable on realistic parsers.
...
\begin{itemize}
    \item \textbf{FieldRestorer:} Undoes the size and value transformations applied by the FieldMinimiser, so that the synthesised state machine refers to the original fields and constants from the input.

    \item \textbf{P4Generator:} Walks the restored state machine and emits the final P4 code, which is accepted as a valid parser by \texttt{p4c}.
\end{itemize}
...
making sure that the state machine produced by synthesis is either the same as the
...
The restrictions on the DSL are what make these optimisations possible.
In particular, the restriction that every condition is over a single field.
...
\begin{itemize}
    \item \textbf{The struct contains at least one compared field.} The
    uncompared fields are dropped for the sake of synthesis. The DSL
    always extracts a struct as a whole and the final P4 code does the
    same, so these fields need not be considered in the synthesiser.

    \item \textbf{The struct contains no compared fields.} Such a struct
    ...
    struct as a single 1-bit field during synthesis. Assigning the field to have 
    ...
    code generation, is discussed in the FieldRestorer.
\end{itemize}
...
Ternary matches are more difficult to handle. A single bit position can take
part in more than one mask, so the renumbering approach from Class~2 does
not apply. Swapping bit values would break some of the masked
conditions.
...
first and they used \texttt{.pc} as input and produced the state-machine
...
\begin{itemize}
    ...
    \item The widths of the fields and the constants have been modified based by the 
    FieldMinimiser.
...
\begin{itemize}
    ...
    \texttt{0} and \texttt{1}. The field is shrunk from size \texttt{16} to size \texttt{2} (Class 2).
...
\begin{itemize}
    \item The number of distinct mask-value pairs needed can grow as
    $O\!\left(\prod_i k_i\right)$, where $k_i$ is the number of
    conditions on the $i$-th subfield. This blows up quickly.

    \item It inherits the ternary handling problem from Class~3.
\end{itemize}
...
Apart from constructing the CFG, the CFGBuilder also performs some
...
starting at the entry node with a packet pointer at the first bit
...
\begin{itemize}
    ...
    PHV along with the True/False bit recorded on this node.
...
where \texttt{default} is the bit-vector \texttt{100} in binary. The
largest field after minimisation is of size 2, so
\texttt{maximum\_field\_size+1} = 3 bits.
...
is used to run a CEGIS loop and synthesise the state machine as a
table.

The design of the state machine is inspired by the P4 parser state
...
motivates the second design and also enables a powerful
optimisation in the FieldRestorer.

The state machine is expressed as a table. Each state has a default
...
When making state transition, the field extracted in that
...
Generator and a Verifier. The Generator produces state machines compliant with the
counterexamples seen so far. 
The Verifier checks if it satisfies the specification for
every possible input packet by trying to a counterexample. 
...
counterexample for the synthesised state machine. 
...
the symbolic simulation of the state machine and the per-packet
...
state machine to be loopless. We accept this restriction since
the input programs are loopless as well. In such a state machine applying transitions
...
\texttt{p + sz > max\_packet\_size}: \textbf{change to >= here and < previously ??}
\begin{lstlisting}[style=codeside, language=C]
g = (cur_state == s) and (default_field[s] == f)
                     and (cur_packet_ptr == p)
...
state machine with priority-ordered transitions is not standard, and
...
taken on every visit. We ignore the default field in this case as it doesn't matter.
We would also have to run an additional graph traversal algorithm to find the state 
which extracts the field after the synthesis as the state machine itself doesn't 
...
original bit-widths of the fields and the renumbering of the
...
The P4Generator walks the restored state machine and emits the
final P4 code. P4 allows only the extraction of whole headers, not
field by field. The synthesised state machine extracts one field
...
This grouping is especially important for the second design. In
that design, fields of a header may not be extracted in the same
order they appear in the source program, so the grouping is what
reconstructs a valid P4 parser.
...
state machine afterwards. This is the optimisation referred to
...
protocol headers, and most of their conditions are exact matches.
...
\draw[arr] (s1) -- (acc);
\end{tikzpicture}
\caption{The synthesised state machine after dropping
...
\texttt{hdr1.field1} is then replicated across the \texttt{== 3}
and \texttt{== 4} branches, since the two branches go on to
extract different headers (\texttt{hdr4} on one, \texttt{hdr5} on
the other). \texttt{hdr2} appears in multiple places in the
stitched machine for similar reasons. The final state machine is
...
\draw[arr] (s0)  -- node[elabel] {default}        (s2d);
...
\caption{The state machine after stitching back \texttt{hdr2},
\texttt{hdr4} and \texttt{hdr5}. The state extracting \texttt{hdr2}
on the default paths is shared.}
\label{fig:stitched-stm}
\end{figure}

This stitched state machine is equivalent to what the synthesiser
would have produced if the uncompared headers had been kept, but
is obtained much faster. The stitching procedure is mechanical and
