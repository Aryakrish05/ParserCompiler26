...
Packet processing code today is written using C(for software packet processors) or using P4(for hardware packet processors). Parsers in P4 directly map to FSMs whereas the control flow in C code implicitly encodes a FSM. A packet-processing program in C and its P4 equivalent are shown in
Figure~\ref{fig:c-vs-p4}. Only the parsing details are shown for brevity. It can be observed that P4 has separate blocks for parsing and processing whereas in
...
It would thus be beneficial to automatically convert sequential C-like code to an equivalent FSM represented in P4. We want both the synthesised FSM and the input C-like code to extract the same sequence of headers for any given packet and make the same final decision of accepting or rejecting the packet.
\section{Objectives}
