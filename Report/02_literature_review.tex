...
(Conflict-Driven Clause Learning)~\cite{cdcl_handbook} to decide
satisfiability of formulas in these theories. DPLL
(Davis-Putnam-Logemann-Loveland) is a classical backtracking
algorithm for SAT that combines case splitting with unit
propagation. CDCL extends DPLL by analysing each conflict during
the search and learning a new clause from it, which prunes the
future search by preventing revisits to similar dead-ends. 
We used Z3's Python API (Z3py) for the project 
and the Z3 documentation was referred to often
...
It also solves a synthesis problem for parsers but compiles P4
code into a TCAM-based hardware implementation. The problem it solves is
different from that of our work as we convert
sequential C-like code into a FSM (similar to the output
of ParserHawk in some ways) and then back to P4. ParserHawk was very useful in
inspiring ideas which shaped our project, mainly the synthesis
encoding and logical specification construction.